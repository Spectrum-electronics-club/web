        ...
        <section style={{ position: 'relative', padding: '7rem 0 3rem', overflow: 'hidden' }}>
          <div className="container-main" style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
              <p style={{ color: '#06b6d4', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1rem' }}>Photo Gallery</p>
          ...
          <div className="container-main">
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '2rem', justifyContent: 'center' }}>
                {albums.map(a => (
                  <button key={a} onClick={() => setAlbum(a)} style={{
                    ...
                    background: album===a ? 'rgba(6,182,212,0.1)' : 'transparent',
                    borderColor: album===a ? 'rgba(6,182,212,0.4)' : 'rgba(255,255,255,0.08)',
            ...
            {loading && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: '1rem' }}>
                {[1,2,3,4,5,6,8,9,10,11,12].map(i => <div key={i} className="skeleton" style={{ height: '200px', borderRadius: '10px' }} />)}
              </div>
            )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: '1rem' }}>
                {filtered.map((img, i) => (
                  <button key={img._id}
                    ...
                    style={{ border: 'none', padding: 0, cursor: 'pointer', borderRadius: '10px', overflow: 'hidden',
                      background: 'transparent', display: 'block', position: 'relative' }}>
                    <img src={img.imageUrl} alt={img.caption || ''} loading="lazy"
                      style={{ width: '100%', height: '200px', objectFit: 'cover', display: 'block',
                        transition: 'transform 0.3s', borderRadius: '10px',
          ...
          <div role="dialog" aria-modal="true" aria-label="Image viewer"
            ...
            <div onClick={e => e.stopPropagation()} style={{ maxWidth: '900px', width: '100%', textAlign: 'center' }}>
              <img src={filtered[lb].imageUrl} alt={filtered[lb].caption || ''}
                style={{ maxWidth: '100%', maxHeight: '80vh', objectFit: 'contain', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.08)' }} />
              {filtered[lb].caption && (
