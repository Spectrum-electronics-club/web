        ...
        <section style={{ position: 'relative', padding: '7rem 0 3rem', overflow: 'hidden' }}>
          <div className="container-main" style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
              <p style={{ color: '#06b6d4', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1rem' }}>Get In Touch</p>
              ...
              <div className="card-glass" style={{ padding: '2rem' }}>
                <form onSubmit={submit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  <div style={fieldStyle}>
                    ...
                    <input id="contact-name" className={`input-dark${errors.fullName?' error':''}`} value={fields.fullName} onChange={ch('fullName')} placeholder="Your name" />
                    {errors.fullName && <span style={errStyle}>{errors.fullName}</span>}
                  </div>
                  <div style={fieldStyle}>
                    <label htmlFor="contact-email" style={labelStyle}>Email *</label>
                    <input id="contact-email" className={`input-dark${errors.email?' error':''}`} type="email" value={fields.email} onChange={ch('email')} placeholder="you@example.com" />
                    {errors.email && <span style={errStyle}>{errors.email}</span>}
                  </div>
                  <div style={fieldStyle}>
                    <label htmlFor="contact-subject" style={labelStyle}>Subject *</label>
                    <input id="contact-subject" className={`input-dark${errors.subject?' error':''}`} value={fields.subject} onChange={ch('subject')} placeholder="What is this about?" />
                    {errors.subject && <span style={errStyle}>{errors.subject}</span>}
                  </div>
                  <div style={fieldStyle}>
                    <label htmlFor="contact-message" style={labelStyle}>Message *</label>
                    <textarea id="contact-message" className={`input-dark${errors.message?' error':''}`} value={fields.message} onChange={ch('message')} rows={5} placeholder="Your message…" style={{ resize: 'vertical' }} />
