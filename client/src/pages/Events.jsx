  ...
  return (
    <div style={{ display: 'flex', gap: '0.4rem' }}>
      {[['d', 'Days'], ['h', 'Hrs'], ['m', 'Min'], ['s', 'Sec']].map(([k, l]) => (
        <div key={k} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '8px', padding: '0.4rem 0.6rem', textAlign: 'center', minWidth: '46px' }}>
          <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 800, fontSize: '1.1rem', color: '#22d3ee' }}>{t[k] ?? 0}</div>
        ...
        <section style={{ position: 'relative', padding: '7rem 0 3rem', overflow: 'hidden' }}>
          <div className="container-main" style={{ position: 'relative', zIndex: 1 }}>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
              <p style={{ color: '#06b6d4', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1rem' }}>Events & Competitions</p>
        ...
        <section style={{ padding: '2rem 0 6rem' }}>
          <div className="container-main">
            ...
            {!loading && !error && events.map((ev) => (
              <div key={ev._id} style={{ marginBottom: '1.25rem' }}>
                          ...
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                            {ev.speakers.map((sp, j) => (
                              <button key={j} onClick={() => setSpeaker(sp)} style={{
                                background: 'rgba(6,182,212,0.06)', border: '1px solid rgba(255,255,255,0.08)',
          ...
          <div role="dialog" aria-modal="true" onClick={() => setSpeaker(null)}
            ...
            <div onClick={e => e.stopPropagation()} className="card-glass" style={{ maxWidth: '420px', width: '100%', padding: '2rem' }}>
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                {speaker.photo && <img src={speaker.photo} alt={speaker.name} style={{ width: '60px', height: '60px', borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(255,255,255,0.1)', flexShrink: 0 }} />}
                <div>
