        ...
        <section style={{ position: 'relative', padding: '7rem 0 3rem', overflow: 'hidden' }}>
          <div className="container-main" style={{ position: 'relative', zIndex: 1 }}>
            <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4 }}>
              <p style={{ color: '#06b6d4', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1rem' }}>Our Work</p>
          ...
          <div className="container-main">
              ...
              <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap' }}>
                {STATUS.map(s => (
                  <button key={s} onClick={()=>setStatus(s)} style={{
                    ...
                    background: status===s ? 'rgba(6,182,212,0.1)' : 'transparent',
                    borderColor: status===s ? 'rgba(6,182,212,0.4)' : 'rgba(255,255,255,0.08)',
            ...
            {loading && (
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:'1.5rem' }}>
                {[1,2,3,4,5,6].map(i => <div key={i} className="skeleton" style={{ height:'240px', borderRadius:'12px' }} />)}
              </div>
            )}
            ...
            {!loading && !error && filtered.length > 0 && (
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:'1.5rem' }}>
                {filtered.map((p) => (
                  <Link key={p._id} to={`/projects/${p._id}`} style={{ textDecoration:'none', display:'block', height:'100%' }}>
                    <div className="card-glass" style={{ padding:'1.5rem', height:'100%', display:'flex', flexDirection:'column', gap:'0.75rem' }}>
                      {p.images?.[0] && <img src={p.images[0]} alt={p.title} loading="lazy" style={{ width:'100%', height:'160px', objectFit:'cover', borderRadius:'8px', marginBottom:'0.25rem' }} />}
                      ...
                      <div style={{ display:'flex', gap:'0.75rem', paddingTop:'0.5rem', borderTop:'1px solid rgba(255,255,255,0.04)' }}>
